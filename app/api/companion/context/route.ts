...
export async function GET(req: NextRequest) {
  ...
  try {
    const [balRes, commRes, envRes, incRes, fundMovsRes] = await Promise.all([
      ...
      supabase.from('finance_envelopes').select('id,label,target').eq('scope', 'personal').eq('archived', false).order('sort_order'),
    ...
    // Per-fund saved (flow-aware). "Guardado" = Σ of every ACTIVE 'personal' fund. envRes is already
    // filtered to active personal funds; Uptown/archived ones aren't in it. Replaces the now-frozen
    // finance_balance.caja_fuerte snapshot column.
    const personalIds = new Set((envRes.data ?? []).map((e: { id: string }) => e.id))
    ...
    for (const m of (fundMovsRes.data ?? []) as Array<{ envelope_id: string | null; amount: number; flow: string }>) {
      if (m.envelope_id && personalIds.has(m.envelope_id)) savedByEnv[m.envelope_id] = (savedByEnv[m.envelope_id] ?? 0) + (m.flow === 'out' ? Number(m.amount) : -Number(m.amount))
    }
    const guardado = Object.values(savedByEnv).reduce((s, v) => s + v, 0)
