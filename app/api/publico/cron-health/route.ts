...
const EMPTY_DAYS: Record<string, number> = { clip: 10, propinas: 7, compras: 14 }   // sin entrada = no se alerta por vacío
const STALE_H = 36
...
function stateOf(r: Row, now: number): { state: State; detail: string } {
  ...
  if (r.last_error) return { state: 'error', detail: r.last_error.slice(0, 120) }
