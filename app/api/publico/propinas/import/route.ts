...
import { importPosterTips, runPosterTipsImport } from '@/lib/posterTips'
...
export async function GET(req: NextRequest) {
  ...
  const to = todayMX(), from = req.nextUrl.searchParams.get('from') ?? shiftDays(to, -7)
  const dry = await importPosterTips(supabase, { from, to, commit: false }).catch(() => null)
  return NextResponse.json({ sync: sync ?? null, dryRun: dry })
}

// POST → COMMIT. body { from?, to? }. Fuente Poster (tips_cash + tips_card).
export async function POST(req: NextRequest) {
  ...
  const from = b.from && /^\d{4}-\d{2}-\d{2}$/.test(b.from) ? b.from : shiftDays(to, -7)
  const supabase = createServerClient()
  const r = await runPosterTipsImport(supabase, from, to)
