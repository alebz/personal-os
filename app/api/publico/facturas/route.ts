...
export async function GET(req: NextRequest) {
  ...
  const facturas = (data ?? [])
    .map((f) => { const fp = formaPagoInfo(f.xml ? parseCFDI(f.xml as string).formaPago : null); const { xml, ...rest } = f; void xml; return { ...rest, formaPagoLabel: fp.label, origenSugerido: fp.origen } })
  ...
  return NextResponse.json({ facturas, pendientes })
}
