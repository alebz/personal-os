...
export async function POST(req: NextRequest) {
  let b: { previsto_id?: string; ocurrencia?: string }
  ...
  const { data: costo, error: cErr } = await supabase.from('publico_costos').insert({
    ...
    category: p.categoria, cost_kind, origin: p.origin ?? null, amount: p.amount, note: p.concepto,
  }).select('id').single()
  if (cErr) return NextResponse.json({ error: cErr.message }, { status: 500 })

  const { error: pErr } = await supabase.from('publico_previsto_pagos').insert({ previsto_id: b.previsto_id, ocurrencia: b.ocurrencia, costo_id: costo.id })
