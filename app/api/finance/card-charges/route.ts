...
export async function POST(req: NextRequest) {
  let b: { card_id?: string; name?: string; amount?: number; meses?: number; start_month?: string; kind?: string; attribution?: string | null }
  ...
  const row = {
    ...
    kind, attribution, sort_order: (last?.sort_order ?? -1) + 1,
  }
