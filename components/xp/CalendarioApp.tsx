...
function EventDialog({ date, marker, onClose, onSaved }: { date: string; marker?: Marker; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div onMouseDown={onClose} style={{ position: 'absolute', inset: 0, zIndex: 50, background: 'rgba(20,40,80,0.28)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onMouseDown={(e) => e.stopPropagation()} style={{ width: 320, maxWidth: '92%', background: '#ece9d8', border: '1px solid #0a3a86', borderRadius: 4, boxShadow: '0 6px 22px rgba(0,0,0,0.35)', overflow: 'hidden', fontSize: 11 }}>
        <div style={{ background: `linear-gradient(180deg,#2b5fb0,#1c4790)`, color: '#fff', fontWeight: 700, padding: '4px 9px', display: 'flex' }}><span style={{ flex: 1 }}>{editing ? 'Editar evento' : 'Nuevo evento'}</span><button onClick={onClose} style={{ border: 0, background: 'rgba(255,255,255,0.15)', color: '#fff', width: 16, height: 16, borderRadius: 2, cursor: 'pointer', lineHeight: 1 }}>×</button></div>
