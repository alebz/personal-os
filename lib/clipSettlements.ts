...
export async function importClipSettlements(supabase: SupabaseClient, opts: { from: string; to: string; commit: boolean }): Promise<ClipResult> {
  ...
  const res = await fetch(`${API}?from=${opts.from}&to=${opts.to}`, {
    headers: { 'x-api-key': token, 'Authorization': `Basic ${token}`, 'Accept': 'application/vnd.com.payclip.v2+json' },
    cache: 'no-store',
  }).catch((e) => ({ ok: false, status: 0, text: async () => String(e) } as unknown as Response))
  if (!res.ok) return { ok: false, error: `Clip ${res.status}: ${(await res.text().catch(() => '')).slice(0, 160)}`, status: res.status || 502 }
